...
bool Bitmap::LoadTexture(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* filename)
{
	...
	bool isDdsFile;
	isDdsFile = StringHelper::ends_with(fileLocation, std::string("dds"));
	if (!isDdsFile) {
		isDdsFile = StringHelper::ends_with(fileLocation, std::string("DDS"));
	}
	...
	if (isDdsFile) {
		result = m_Texture->InitializeDDS(device, StringHelper::getWChar(filename));
	}
	else {
...
void Bitmap::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	// SetContext the index buffer to active in the input assembler so it can be rendered.
	deviceContext->IASetIndexBuffer(m_indexBuffer, DXGI_FORMAT_R32_UINT, 0);

	// SetContext the type of primitive that should be rendered from this vertex buffer, in this case triangles.
