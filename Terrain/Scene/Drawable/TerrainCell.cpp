...
bool TerrainCell::BuildLineBuffers(ID3D11Device* device)
{
	...
	index = 0;
	increment = m_maxHeight - m_minHeight;

	for (float currentHeight = m_minHeight; currentHeight <= m_maxHeight; currentHeight +=increment) {
		// 8 Horizontal lines.
		vertices[index].position = XMFLOAT3(m_minWidth, currentHeight, m_minDepth);
		vertices[index].color = lineColor;
		indices[index] = index;
		index++;

		vertices[index].position = XMFLOAT3(m_maxWidth, currentHeight, m_minDepth);
		vertices[index].color = lineColor;
		indices[index] = index;
		index++;

		vertices[index].position = XMFLOAT3(m_minWidth, currentHeight, m_maxDepth);
		vertices[index].color = lineColor;
		indices[index] = index;
		index++;

		vertices[index].position = XMFLOAT3(m_maxWidth, currentHeight, m_maxDepth);
		vertices[index].color = lineColor;
		indices[index] = index;
		index++;
...
void TerrainCell::CalculateCellDimensions()
{
	...
	m_positionX = (m_maxWidth - m_minWidth) /2 + m_minWidth;
	m_positionY = (m_maxHeight - m_minHeight) /2 + m_minHeight;
	m_positionZ = (m_maxDepth - m_minDepth) /2 + m_minDepth;
