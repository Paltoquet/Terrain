...
bool Font::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* fontFilename, WCHAR* textureFilename,
...
bool Font::LoadTexture(ID3D11Device* device, ID3D11DeviceContext* deviceContext, WCHAR* filename)
{
	bool result;
	...
	result = m_Texture->InitializeDDS(device, filename);
	if(!result)
	{
		return false;
	}
